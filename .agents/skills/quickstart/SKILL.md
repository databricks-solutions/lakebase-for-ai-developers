...
>
> **Tool choice:**
> - Use `request_user_input` for **multi-choice** questions when it is available. If it is not available, ask the same question directly. The steps below mark structured questions explicitly.
> - For **free-text follow-ups** (a name, URL, endpoint path), ask in a normal chat message and wait for the user's reply. Do not manufacture choices for a free-text answer.

### Step 1: Workspace / Databricks profile

1. Run `databricks auth profiles` to list configured profiles.
2. **[Structured user input]**: **"Which Databricks workspace do you want to use?"**
...
**[Structured user input]**: **"Do you have an existing Databricks app to deploy to?"**
...
Lakebase is required. **[Structured user input]**: **"How will you provide Lakebase for agent memory?"**
...
Parse the JSON response to find an endpoint (the field is `endpoints[].name`; typical default is `primary`). If there's exactly one, use it. If there are multiple, use **structured user input** with each endpoint name as an option for the user to pick. Construct the full resource path `projects/<project>/branches/<branch>/endpoints/<endpoint-name>` and record `--lakebase-autoscaling-endpoint <constructed-path>` for Step 4. The script never sees project+branch separately — it gets a fully-formed endpoint resource path.
...
Lakebase is optional, for chat-UI history. **[Structured user input]**: **"Set up Lakebase for chat-UI history?"**
...
If you don't pass a flag, the script falls back to interactive `input()` prompts. For non-interactive use (Codex / CI / scripted setup), pre-supply the flag that corresponds to each prompt — otherwise the subprocess hangs on stdin and eventually exits with `EOFError`.
...
| Prompt | When it fires | Skip with |
|---|---|---|
| `Enter the existing app name to bind to (or press Enter to skip):` | No `--app-name` flag AND stdin is a TTY (Codex / CI auto-skip via `isatty()` check) | `--app-name <name>` |
...
For Codex / CI to run end-to-end without hanging:
