...
`AGENTS.md`, `CLAUDE.md`) links to it; don't scatter duplicate diagrams elsewhere.
...
These recur — check each across `AGENTS.md`, `CLAUDE.md`, `README.md`, `agent_server/README.md`,
`data/README.md`, and `docs/**`. Each is "what the docs tend to say" → "what's actually true;
verify, don't assume":
...
`.agents/skills/` is mostly **Codex-ported + SHA-pinned** upstream copies (see
...
- **Stale markers gone** — grep the tracked docs (exclude `.agents/skills/` ported bodies):
