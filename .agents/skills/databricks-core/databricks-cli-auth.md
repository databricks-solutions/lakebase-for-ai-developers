...
**CRITICAL**: When working in Codex, each Bash command executes in a **separate shell session**. This has important implications for profile management:
...
2. **If profiles exist:**
   ...
   - **In Codex, always use the `--profile` flag** rather than setting environment variables
...
**IMPORTANT FOR CODEX USERS**: In Codex, each Bash command runs in a **separate shell session**. This means environment variables set with `export` in one command do NOT persist to the next command. See the Codex-specific guidance below.

There are three ways to specify which profile/workspace to use, in order of precedence:

#### 1. CLI Flag (Highest Priority) - RECOMMENDED FOR CODEX
...
**In Codex, this is the most reliable method** because it doesn't depend on persistent environment variables.
...
**CRITICAL - Codex Users:**

Since each Bash command in Codex runs in a separate shell, you **CANNOT** do this:
...
```bash
# ✅ WORKS in Codex - export and command run in same shell
export DATABRICKS_CONFIG_PROFILE=staging && databricks jobs list
...
# DO NOT use this pattern in Codex
...
**In Codex, use --profile flag with each command (RECOMMENDED):**
```bash
...
**Alternatively in Codex, chain commands with &&:**
```bash
...
**Traditional Terminal Session (for reference only - NOT for Codex):**
```bash
# This pattern works in regular terminals but NOT in Codex
...
**Codex version** (with chained commands):
```bash
