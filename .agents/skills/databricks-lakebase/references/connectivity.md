...
```bash
# .env (add to .gitignore); keep the password in a local secret store or shell variable
LAKEBASE_PG_HOST="<host>"
LAKEBASE_PG_USER="<user>"
LAKEBASE_PG_DATABASE="<database>"
```
...
url = URL.create(
    "postgresql+psycopg",
    username=os.environ["LAKEBASE_PG_USER"],
    password=os.environ["LAKEBASE_PG_PASSWORD"],
    host=os.environ["LAKEBASE_PG_HOST"],
    port=5432,
    database=os.environ["LAKEBASE_PG_DATABASE"],
