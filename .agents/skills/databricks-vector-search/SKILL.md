...
- **[databricks-core](../databricks-core/SKILL.md)** - Discover and inspect the Unity Catalog tables that back Delta Sync indexes
- **[databricks-dabs](../databricks-dabs/SKILL.md)** - Define jobs and pipelines that build Vector Search source tables

Additional upstream Databricks skills such as `databricks-agent-bricks`,
`databricks-unstructured-pdf-generation`, `databricks-unity-catalog`, and
`databricks-pipelines` are not included in this repo. Install them explicitly if the task requires
those workflows.
